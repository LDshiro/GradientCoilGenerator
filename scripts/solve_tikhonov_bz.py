...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(description="Solve Bz inverse problem with Tikhonov.")
    ...
    parser.add_argument("--reg-operator", choices=["identity", "grad", "power"], default="grad")
    parser.add_argument("--gradient-rows-reg", choices=["interior", "active"], default="interior")
    parser.add_argument("--r-sheet", type=float, default=1.0)
    ...
    spec = TikhonovBzSpec(
        ...
        reg_operator=str(args.reg_operator),
        r_sheet=float(args.r_sheet),
        gradient_rows_reg=str(args.gradient_rows_reg),
    ...
    out_path = out_dir / f"tikhonov_{stamp}.npz"
    ...
    config = {
        ...
        "lambda_reg": float(args.lambda_reg),
        "reg_operator": args.reg_operator,
        "gradient_rows_reg": args.gradient_rows_reg,
        "r_sheet": float(args.r_sheet),
        "cg_tol": float(args.cg_tol),
        "cg_maxiter": int(args.cg_maxiter),
